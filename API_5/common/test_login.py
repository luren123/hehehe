...
#获取测试用例
test_Register = DoExcsl(project_path.case_path,"Sheet3").login_excel()
# print(test_Register)
#实例化logging
logger = Mylog()
# print(test_Register)
...
class TestHttpRequest(unittest.TestCase):  #定义一个类
    def setUp(self):
        self.t = DoExcsl(project_path.case_path,"Sheet3")
    ...
    def test_001(self,L):
            ...
            #执行http请求
            res = jikoufengzhuang().http_requests(L["url"],param=eval(L["Params"]),method=L["Method"],cookies=COOKIES)  #实际结果
            print("用例执行的结果是：{}".format(res.text))
            try:
                self.assertEqual(eval(L["ExpectedResult"]),res.json())
