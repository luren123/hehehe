...
from jiekou.用例封装 import DoExcsl #导入类
from jiekou.接口日志 import Mylog
from Common import  project_path11  #导入用例的绝对路径
from jiekou.充值接口封装 import jikoufengzhuang
#获取测试用例
test_Register = DoExcsl(project_path11.conf_path,"withdraw").recharge_test()
#实例化logging
logger = Mylog()
COOKIES = None  #设置初始值
# print(test_Register)
...
class TestHttpRequest(unittest.TestCase):  #定义一个类
    def setUp(self):
        self.t = DoExcsl(project_path11.conf_path,"racharge")
    ...
    def test_001(self,L):
            ...
            if res.cookies:  #判断请求的cookies是否为空  不为空就是ture
                COOKIES = res.cookies
            try:
                self.assertEqual(eval(L["ExpectedResult"])["code"],res.json()["code"])
                #根据状态码可以解决不好判断的情况
